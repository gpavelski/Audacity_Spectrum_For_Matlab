...
alg = 4; %Best: Cepstrum with Hamming Window, Enhanced Autocorrelation with Gaussian 2.5 window
windowFunc = 8;
windowSize = 2^10;
LogPlot = 0;
...
Pfactor = 1;
Piecesize = Pfactor*windowSize;
npieces = floor(length(data)/Piecesize);

M = zeros([windowSize/2,npieces]);
[Points, SinTable, BitReversed] = InitializeFFT(windowSize);

for i = 1:npieces
    datapiece = data((i-1)*Piecesize+1: i*Piecesize);
    M(:,i) = SpectrumAnalyst(Algorithm(alg), eWindowFunctions(windowFunc), windowSize, fs, datapiece, Piecesize, Points, SinTable, BitReversed);
end
...
colormap(jet(256));
imagesc([0,length(data)/fs], [-t(ifmin), -t(ifmax)] ,M(ifmin:-1:ifmax,:));
hold on; % Hold the plot

%  Plot the notes lines
for i = ind1:ind2
    s = plot(linspace(0,length(data)/fs,10), -1./fnotes(i).*ones(1, 10) , '-w');
...
% 
yticks(-1./fnotes(ind1:ind2)); % Redefine the y-axis ticks as the frequencies of the musical notes
